...
if ($action == 'create_invoice'){
	...
	// invoice customer information
	// billing
	$customer_id= $_POST['customer_id']; // customer name
	...
	if($mysqli -> multi_query($query)){
		...
		//Set to
		$invoice->setTo(array($customer_id,$customer_name));
		//,$customer_address_1,$customer_address_2,$customer_town,$customer_county,$customer_postcode,"Phone: ".$customer_phone));
		//Ship to
...
if($action == 'login') {
    ...
    if($count!="") {
		...
		echo json_encode(array(
			'status' => 'Success',
			'message'=> 'সফলভাবে লগইন হয়েছে! অনুগ্রহ করে অপেক্ষা করুন!'
		));
    } else {
    	echo json_encode(array(
	    	'status' => 'Error',
	    	//'message'=> 'There has been an error, please try again.'
	    	'message' => 'ব্যবহারকারীর নাম/পাসওয়ার্ড ভুল! দয়াকরে আবারে চেষ্টা করুন!'
	    ));
    }
}
...
if($action == 'add_area') {
	...
	$area_code = eng_num($_POST['area_code']);
	$area_name = $_POST['area_name'];
	$area_details = $_POST['area_details'];
	$user_name = $_POST[user_name];
	$create_date = date("Y/m/d");
	//our insert query query
	$query  = "INSERT INTO AreaCreate
				(
					AreaCode,
					AreaName,
					AreaDetails,
	...
	$stmt = $mysqli->prepare($query);
	if($stmt === false) {
	  trigger_error('ভুল SQL: ' . $query . ' ভুল: ' . $mysqli->error, E_USER_ERROR);
	}

	//$user_password = md5($user_password);
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('sssss',$area_code,$area_name,$area_details,$create_date,$user_name);

	if($stmt->execute()){
	    //if saving success
		echo json_encode(array(
			'status' => 'সাফল্য',
			'message'=> 'এলাকা সফলভাবে যোগ করা হয়েছে!'
		));
	    ...
	    echo json_encode(array(
	    	'status' => 'ভুল',
	    	//'message'=> 'There has been an error, please try again.'
	    	'message' => 'একটি সমস্যা হয়েছে, আবার চেষ্টা করুন। <pre>'.$mysqli->error.'</pre><pre>'.$query.'</pre>'
	    ));
	}
...
if($action == 'add_member') {
	$member_id= eng_num($_POST['member_id']);
	$member_name = $_POST['member_name'];
	$fahters_name = $_POST['fathers_name'];
	$age = eng_num($_POST['age']);
	$address = $_POST['address'];
	$mobile = eng_num($_POST['member_mobile']);
	$nationality = $_POST['nationality'];
	$occuption = $_POST['occuption'];
	$nominated_member = $_POST['nominated_member'];
	$nominated_member_relation = $_POST['nominated_member_relation'];
	$nominated_member_address = $_POST['nominated_member_address'];
	$nominated_member_mobile = eng_num($_POST['nominated_member_mobile']);
	$area_code = eng_num($_POST['area_code']);
	$t_date = $_POST['member_date'];
	$member_date = preg_replace("/(\d+)\D+(\d+)\D+(\d+)/","$3-$2-$1",$t_date);
	$user_name = $_POST[user_name];
	$create_date = date("Y/m/d");
	
	//our insert query query
	$query  = "INSERT INTO MemberCreate
				(
					MemberID,
					MemberName,
					FathersName,
	...
	//$user_password = md5($user_password);
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('ssssssssssssssss',$member_id, $member_name, $fahters_name, $age, $address, $mobile, $nationality, $occuption, $nominated_member, $nominated_member_address, $nominated_member_mobile, $nominated_member_relation, $member_date, $area_code, $user_name, $create_date);

	if($stmt->execute()){
	    //if saving success
		echo json_encode(array(
			'status' => 'সাফল্য',
			'message'=> 'সভ্যগন সফলভাবে যোগ করা হয়েছে!'
		));
...
if($action == 'add_cal') {
	$member_id= eng_num($_POST['member_id']);
	$RealizationSavings = eng_num($_POST['realization_savings']);
	$b_savings = eng_num($_POST['Savings_Balance']);
	$w_savings = eng_num($_POST['Savings_Withdrawal']);
	$Others_withdrawal = eng_num($_POST['Others_withdrawal']);
	
	$invenstment_recovery = eng_num($_POST['invenstment_recovery']);
	$b_investment = eng_num($_POST['b_investment']);
	$investment = eng_num($_POST['investment']);
	$others_investment = eng_num($_POST['others_investment']);
	...
	//$user_password = md5($user_password);
	/* Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob */
	$stmt->bind_param('sssssssssss',$member_id, $RealizationSavings, $b_savings, $w_savings, $Others_withdrawal, $invenstment_recovery, $investment, $b_investment, $others_investment, $user_name, $create_date);

	if($stmt->execute()){
	    //if saving success
		echo json_encode(array(
			'status' => 'সাফল্য',
			'message'=> 'সভ্যগনের হিসাব সফলভাবে যোগ করা হয়েছে!'
		));
...
if($action == 'update_area') {
	...
	$stmt = $mysqli->prepare($query);
	if($stmt === false) {
	  trigger_error('ভুল SQL: ' . $query . ' Error: ' . $mysqli->error, E_USER_ERROR);
	}
		...
		$stmt->bind_param(
			'sss',
			$area_name, $area_details, $area_code
		);
	...
	if($stmt->execute()){
		...
		echo json_encode(array(
			'status' => 'সাফল্য',
			'message'=> 'এলাকা সফলভাবে হালনাগাদ করা হয়েছে!'
		));
...
if($action == 'update_member') {
	...
	$nominated_member_mobile = eng_num($_POST['nominated_member_mobile']);
	$area_code = eng_num($_POST['area_code']);
	$t_date = eng_num($_POST['member_date']);
	$member_date = preg_replace("/(\d+)\D+(\d+)\D+(\d+)/","$3-$2-$1",$t_date);
	$user_name = $_POST[user_name];
	$create_date = date("Y/m/d");
	//$m_image = trim( addslashes( $_POST['m_image'] ) ); //addslashes(file_get_contents($_FILES['m_image']['tmp_name'])); //SQL Injection defence!
if(!empty($_FILES['m_image']['tmp_name']) 
     && file_exists($_FILES['m_image']['tmp_name'])) {
    $m_image= addslashes(file_get_contents($_FILES['m_image']['tmp_name']));
}
	
	//$m_image=addslashes (file_get_contents($_FILES['m_image']['tmp_name']));
	//$imagename=$_FILES["m_image"]["name"]; 

		$query = "UPDATE MemberCreate SET
					MemberName = ?,
					FathersName = ?,
					Age = ?,
					Address = ?,
	...
	if($stmt === false) {
	  trigger_error('ভুল SQL: ' . $query . ' Error: ' . $mysqli->error, E_USER_ERROR);
		...
		$stmt->bind_param(
			'sssssssssssssss',
			$member_name, $fahters_name, $age, $address, $mobile, $nationality, $occuption, $nominated_member, $nominated_member_address, $nominated_member_mobile, $nominated_member_relation, $member_date, $area_code,$m_image,$member_id
		);
	...
	if($stmt->execute()){
		...
		echo json_encode(array(
			'status' => 'সাফল্য',
			'message'=> 'সভ্যগন সফলভাবে হালনাগাদ করা হয়েছে!'
		));
